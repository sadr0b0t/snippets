/* files: js/lib/es5-shim.min.js,js/lib/jquery-1.8.3.min.js,js/lib/jquery-ui/jquery.ui.core.min.js,js/lib/jquery-ui/jquery.ui.widget.min.js,js/core/support.js,js/core/console.js,js/core/flags.js,js/jquery/jquery.lj.basicWidget.js,js/basic.js,js/jquery_fn.js,js/livejournal.js,js/core/log.js,js/core/string.js,js/core/track.js,js/core/widget.js,js/core/template.js,js/jquery/jquery.tmpl.min.js,js/jquery/jquery.lj.bubble.js,js/jquery/jquery.storage.js,js/esn.js,js/journal.js,js/hourglass.js,js/livejournal-local.js,js/pagescroller.js,js/ippu.js,js/lj_ippu.js,js/ljwidget.js,js/ljwidget_ippu.js,js/contextualhover.js 
*/
...
/* file: js/lib/jquery-1.8.3.min.js 
----------------------------------------------------------------------------------*/
...
(function ($) {
	...
	var loaded = false,
		queue = [],
		timer = Site.timer || +(new Date()),
		stime = Site.server_time || timer,
		prefix = Site.logprefix + '/kaldr.frame#',
		frame, send;
	...
	if (!Site.logprefix) {
		if (typeof LJ.console.log === 'function') {
			LJ.info = LJ.console.info.bind(LJ.console);
			LJ.warn = LJ.console.warn.bind(LJ.console);
			LJ.error = LJ.console.error.bind(LJ.console);
		} else {
			// IE8 case, see 'callable object'

			LJ.info = LJ.console.info;
			LJ.warn = LJ.console.warn;
			LJ.error = LJ.console.error;
		}

		return;
	}

	send = LJ.threshold(function () {
		...
		args.unshift(stime, new Date() - timer);
		frame.attr('src', prefix + args.join(' '));
	...
	function log (level, message) {
		queue.push(['[' + level + ']', location.pathname, message]);

		switch (level) {
			case 'error':
			case 'warn':
				LJ.console[level](message);
				break;
		}
	 ...
	 * @param {message}
	 */
	LJ.info = log.bind(null, 'info');

	/**
	 * Log warning
	 * @param {message}
	 */
	LJ.warn = log.bind(null, 'warn');

	/**
	 * Log error
	 * @param {message}
	 */
	LJ.error = log.bind(null, 'error');
...
(function ($, window) {
	...
	LJ.UI.initWidgetNode = function (node, widget, entryPoint) {
		if (node.hasClass(baseClass)) {
			LJ.warn('Node already has class ' + baseClass);
			return;
		}
	...
	LJ.UI.removeWidget = function (node) {
		...
		if (!node.is(baseClass)) {
			LJ.warn('Widget was not found on node');
			return;
		}

		if (widgets.hasOwnProperty(id)) {
			LJ.warn('Widget ' + id + ' was removed already or never created');
			return;
		}
	...
	LJ.UI.loadTemplate = function (name, callback) {
		...
		$.ajax({
			...
			if (LJ.UI._templates[name]) {
				...
				LJ.warn('Failed to setup template ' + name);
			}
		}).error(function () {
			callback(new Error('Network error'), name);
			LJ.warn('Failed to load template ' + name);
		});
	};
		...
		switch (type) {
			case 'JQuery':
				...
				if (node.length) {
					...
					LJ.warn('Template node #' + data + ' was not found');
					return;
				}
