...
// Расширенный пример - добавлены команды для обратной связи: ping и ledstatus
...
const char* CMD_LEDSTATUS = "ledstatus";

// Ответы для Сервера Роботов
const char* REPLY_OK = "ok";
const char* REPLY_DONTUNDERSTAND = "dontunderstand";
const char* REPLY_LEDON = "on";
const char* REPLY_LEDOFF = "off";

// Пин для тестовой лампочки
#define LED_PIN 13
// статус тестовой лампочки (1: горит/'on', 0: не горит/'off')
int led_status;
...
int handleInput(char* buffer, int buffer_size, char* reply_buffer) {
    ...
    if(strcmp(buffer, CMD_LEDON) == 0) {
        ...
        strcpy(reply_buffer, REPLY_OK);
        replySize = strlen(reply_buffer);
    } else if (strcmp(buffer, CMD_LEDSTATUS) == 0) {
        Serial.println("Command 'ledstatus': get led status");
                
        // Подготовить ответ
        if(led_status == 1) {
            // лампочка включена
            strcpy(reply_buffer, REPLY_LEDON);
        } else {
            // лампочка выключена
            strcpy(reply_buffer, REPLY_LEDOFF);
        }
        replySize = strlen(reply_buffer);
    } else {      
...
void setup() {
    Serial.begin(9600);
    Serial.println("Start WiFi network and Tcp client slave demo2");
